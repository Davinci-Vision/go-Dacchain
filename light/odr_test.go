...
import (
	...
	"github.com/Dacchain/go-Dacchain/consensus/dpos"
)
...
type testOdr struct {
	OdrBackend
	sdb, ldb dacdb.Database
	disable  bool
}

func (odr *testOdr) Database() dacdb.Database {
	return odr.ldb
}
...
type odrTestFn func(ctx context.Context, db dacdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes1(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db dacdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db dacdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrAccounts(ctx context.Context, db dacdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db dacdb.Database, bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func testChainGen(i int, block *core.BlockGen) {
	signer := types.NewDavinciSigner(new(big.Int))
	switch i {
	case 0:
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	var (
		sdb, _  = dacdb.NewMemDatabase()
		ldb, _  = dacdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(sdb)
	)
	gspec.MustCommit(ldb)
	// Assemble the test environment
	blockchain, _ := core.NewBlockChain(sdb, params.TestChainConfig, dac.CreateDavinciConsensusEngine(), vm.Config{},nil)
