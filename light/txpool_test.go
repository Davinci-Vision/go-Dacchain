...
func TestTxPool(t *testing.T) {
	for i := range testTx {
		testTx[i], _ = types.SignTx(types.NewTransaction(uint64(i), acc1Addr, big.NewInt(10000), params.TxGas, nil, nil, 0, nil, nil, nil, nil, ""), types.NewDavinciSigner(new(big.Int)), testBankKey)
	}
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, dac.CreateDavinciConsensusEngine(),  sdb, poolTestBlocks, txPoolTestChainGen)
