...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb, _  = dacdb.NewMemDatabase()
		lightdb, _ = dacdb.NewMemDatabase()
		gspec      = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis    = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, params.TestChainConfig, dac.CreateDavinciConsensusEngine(), vm.Config{},nil)
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis,dac.CreateDavinciConsensusEngine(), fulldb, 4, testChainGen)
