...
import (
	...
	"github.com/Dacchain/go-Dacchain/common/bitutil"
...
func GetChtRoot(db dacdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db dacdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(ChtFrequency/ChtV1Frequency)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db dacdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db dacdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := dacdb.NewTable(db, ChtTablePrefix)
	idb := dacdb.NewTable(db, "chtIndex-")
...
func GetBloomTrieRoot(db dacdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db dacdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db dacdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := dacdb.NewTable(db, BloomTrieTablePrefix)
	idb := dacdb.NewTable(db, "bltIndex-")
