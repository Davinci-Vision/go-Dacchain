...
import (
	...
	"fmt"
	"github.com/Dacchain/go-Dacchain/p2p"
	"sync"
	"github.com/Dacchain/go-Dacchain/core/types"
	"github.com/Dacchain/go-Dacchain/log"
	"math/big"
	"time"
	"github.com/Dacchain/go-Dacchain/dac/downloader"
	"github.com/Dacchain/go-Dacchain/dac/fetcher"
	"github.com/Dacchain/go-Dacchain/event"
	"github.com/Dacchain/go-Dacchain/consensus"
	"github.com/Dacchain/go-Dacchain/p2p/discover"
	"errors"
	"sync/atomic"

	"github.com/Dacchain/go-Dacchain/accounts/walletType"
	"strings"
	"crypto/ecdsa"
...
// NewProtocolManager returns a new Davinci sub protocol manager. The Davinci sub protocol manages peers capable
// with the Davinci network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb dacdb.Database, taskManager *DposTaskManager, blockChan chan *types.Block, addDelegateWalletCallback func(data *aa.DelegateWalletInfo), delegateWallets map[string]*ecdsa.PrivateKey) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		if mode == downloader.FastSync && version < dac02 {
			continue
		}
...
// handle is the callback invoked to manage the life cycle of an dac peer. When
// this function terminates, the peer is disconnected.
func (pm *ProtocolManager) handle(p *peer) error {
	if pm.peers.Len() >= pm.maxPeers {
		return p2p.DiscTooManyPeers
	}
	p.Log().Debug("Davinci peer connected", "name", p.Name())

	// Execute the Davinci handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("Davinci handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Davinci peer registration failed", "err", err)
		return err
	}
	...
	if p.netType == discover.ConsNet && pm.delegatePeers.Peer(p.id) == nil {
		if err := pm.delegatePeers.Register(p); err != nil {
			p.Log().Error("Davinci delegate peer registration failed", "err", err)
			return err
		}
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Davinci message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= dac02 && msg.Code == GetNodeDataMsg:
		return pm.dealGetNodeDataMsg(msg, p)
	case p.version >= dac02 && msg.Code == NodeDataMsg:
		return pm.dealNodeDataMsg(msg, p)
	case p.version >= dac02 && msg.Code == GetReceiptsMsg:
		return pm.dealGetReceiptsMsg(msg, p)
	case p.version >= dac02 && msg.Code == ReceiptsMsg:
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Davinci peer", "peer", id)

	// Unregister the peer from the downloader and Davinci peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Dacchain protocol")
	...
	log.Info("Dacchain protocol stopped")
}
...
func (pm *ProtocolManager) txBroadcastLoop() {
	for {
		...
		case txPreEvent := <-pm.txCh:
			// Add log
			//log.Debug("交易事件写入到管道中，准备广播", "交易信息：", txPreEvent.Tx)
...
func (pm *ProtocolManager) BroadcastTx(hash common.Hash, tx *types.Transaction) {
	...
	//log.Debug("开始广播交易", "不知道该交易的节点数为", len(peers))
	for _, peer := range peers {
...
type NodeInfo struct {
	Network uint64              `json:"network"` // Dacchain network ID (1=Frontier)
