...
// PeerInfo represents a short summary of the Davinci sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"` // Davinci protocol version negotiated
...
func (p *peer) SendTransactions(txs types.Transactions) error {
	...
	//log.Debug("广播交易", "广播交易完毕|trx Length:", len(txs))
...
// Handshake executes the dac protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("dac/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Davinci sub-protocol.
type peerSet struct {
