...
import (
	...
	aa "github.com/Dacchain/go-Dacchain/accounts/walletType"
...
func NewDposMiner(config *params.ChainConfig, dac Backend, engine consensus.Engine) *DposMiner {
	...
	produceBlockCallback := func(ctx context.Context) {
		...
		//pending, err := dposMiner.dac.TxPool().Pending()
		now := time.Now()
...
func (d *DposMiner) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	work := &worker{
		...
		createdAt:  time.Now(),
		tcount:     0,
		signer:     types.NewDavinciSigner(d.config.ChainId),
		header:     header,
