...
func deriveSigner(V *big.Int) Signer {
	if V.Sign() != 0 {
		return NewDavinciSigner(deriveChainId(V))
	}
	return NewDavinciSigner(big.NewInt(1))
}
...
func (tx *Transaction) GetTransactionType() common.Address {
	...
	case ActionTrans:
		a := tx.Asset()
		if nil == a {
			return common.StringToAddress("dac")
		} else {
			return *a
		}
	case ActionRegister:
...
// DacCost returns dac required.
func (tx *Transaction) DacCost() *big.Int {
	log.Info("Transaction|DacCost,", "transaction", tx.data)
