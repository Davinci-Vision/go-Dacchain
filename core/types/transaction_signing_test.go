...
func TestEIP155Signing(t *testing.T) {
	...
	signer := DavinciSigner{chainId:big.NewInt(18)}
...
func TestEIP155ChainId(t *testing.T) {
	...
	tx, err = SignTx(tx, DavinciSigner{}, key)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEIP155SigningVitalik(t *testing.T) {
	...
	for i, test := range []struct {
		...
		signer := DavinciSigner{chainId:big.NewInt(1)}
...
func TestChainId(t *testing.T) {
	...
	tx, err = SignTx(tx, DavinciSigner{chainId:big.NewInt(1)}, key)
	if err != nil {
		t.Fatal(err)
	}

	_, err = Sender(DavinciSigner{chainId:big.NewInt(2)}, tx)
	if err != ErrInvalidChainId {
		t.Error("expected error:", ErrInvalidChainId)
	}

	_, err = Sender(DavinciSigner{chainId:big.NewInt(1)}, tx)
	if err != nil {
