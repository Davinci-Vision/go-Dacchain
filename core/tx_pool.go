...
var (
	...
	ErrInsufficientFunds = errors.New("insufficient funds of DAC")
...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain blockChain) *TxPool {
	...
	pool := &TxPool{
		...
		chain:       chain,
		signer:      types.NewDavinciSigner(chainconfig.ChainId),
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	case types.ActionAddVote, types.ActionSubVote:
		...
		//log.Debug("ValidateTx", "voteCost", voteCost, "lockBalance", lockBalance, "total", new(big.Int).Mul(big.NewInt(params.Dac), big.NewInt(maxElectDelegate)))
		if voteCost.Add(voteCost, lockBalance).Cmp(new(big.Int).Mul(big.NewInt(params.Dac), big.NewInt(maxElectDelegate))) > 0 {
