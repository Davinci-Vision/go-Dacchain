...
import (
	...
	"github.com/Dacchain/go-Dacchain/metrics"
...
var (
	...
	configPrefix   = []byte("davinci-config-") 	// config prefix for the db
...
func WriteCanonicalHash(db dacdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db dacdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db dacdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db dacdb.Putter, hash common.Hash) error {
...
func WriteHeader(db dacdb.Putter, header *types.Header) error {
...
func WriteBody(db dacdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db dacdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db dacdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db dacdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db dacdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db dacdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db dacdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func WriteDelegateBodyRLP(db dacdb.Putter, rlp rlp.RawValue) error {
...
func WriteDelegateShuffleBlockHeightRLP(db dacdb.Putter, rlp rlp.RawValue) error {
...
func PreimageTable(db dacdb.Database) dacdb.Database {
	return dacdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db dacdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db dacdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db dacdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
