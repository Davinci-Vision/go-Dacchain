...
var (
	agentsNumber = flag.Int("number", 101, "代理数量")
	password     = flag.String("password", "yujian", "私钥密码")
	keyStoreDir  = flag.String("keystore-dir", ".", "私钥地址")
)
...
*/
	...
	func mainNetAgents() []types.Candidate {
		return []types.Candidate{ {{range $index, $address := .addressLists}}
			{"{{$address}}",uint64(2000000), "dac-node{{$index}}", 1492009146, }, {{end}}
		}
	}
