...
func GenerateChain(config *params.ChainConfig, parent *types.Block, dacEngine consensus.Engine, db dacdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
...
// header only chain.
func newCanonical(dacEngine consensus.Engine, n int, full bool) (dacdb.Database, *BlockChain, error) {
	// Initialize a fresh chain with only a genesis block
	gspec := new(Genesis)
	db, _ := dacdb.NewMemDatabase()
	genesis := gspec.MustCommit(db)

	blockchain, _ := NewBlockChain(db, params.AllDavinciProtocolChanges, dacEngine, vm.Config{},nil)
...
func makeHeaderChain(parent *types.Header, n int, dacEngine consensus.Engine, db dacdb.Database, seed int) []*types.Header {
...
func makeBlockChain(parent *types.Block, n int, dacEngine consensus.Engine, db dacdb.Database, seed int) []*types.Block {
