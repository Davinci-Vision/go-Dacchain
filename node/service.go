...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (dacdb.Database, error) {
	if ctx.config.DataDir == "" {
		return dacdb.NewMemDatabase()
	}
	db, err := dacdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
