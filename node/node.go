...
import (
	...
	"github.com/Dacchain/go-Dacchain/internal/debug"
...
func (n *Node) OpenDatabase(name string, cache, handles int) (dacdb.Database, error) {
	if n.config.DataDir == "" {
		return dacdb.NewMemDatabase()
	}
	return dacdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
