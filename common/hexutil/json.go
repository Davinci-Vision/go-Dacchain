...
func (b Bytes) MarshalDACText() ([]byte, error) {
...
func bytesHaveDacPrefix(input []byte) bool {
...
func checkNumberText(input []byte) (raw []byte, err error) {
	...
	if !bytesHave0xPrefix(input) && !bytesHaveDacPrefix(input) {
		return nil, ErrMissingPrefix
	}
	if bytesHave0xPrefix(input) {
		input = input[2:]
	} else if bytesHaveDacPrefix(input) {
		input = input[3:]
	}
