...
// Address represents the 20 byte address of an Davinci account.
...
func HexToAddress(s string) Address {
	if hasDACPrefix(s) {
		return BytesToAddress(FromDACHex(s))
	}
	return BytesToAddress(FromHex(s))
}

// IsHexAddress verifies whether a string can represent a valid hex-encoded
// Davinci address or not.
...
func IsDACAddress(s string) bool {
	if hasDACPrefix(s) {
		s = s[3:]
	}
	return len(s) == 2*AddressLength && isHex(s)
...
func (a Address) Hex() string {
	...
	return "DAC" + strings.ToLower(string(result))
}

func (a Address) DacHex() string {
	unchecksummed := hex.EncodeToString(a[:])
	sha := sha3.NewKeccak256()
	sha.Write([]byte(unchecksummed))
	hash := sha.Sum(nil)
...
func (a Address) MarshalText() ([]byte, error) {
	return hexutil.Bytes(a[:]).MarshalDACText()
}
